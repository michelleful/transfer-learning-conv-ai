...
def get_data_loaders(args, tokenizer):
    ...
    for dataset_name, dataset in chat.items():
        num_candidates = len(dataset[0]["candidates"])
        if args.num_candidates > 0 and dataset_name == 'train':
            num_candidates = min(args.num_candidates, num_candidates)
        for dialog in dataset:
            history = dialog["history"][-(2*args.max_history+1):]
            for j, candidate in enumerate(dialog["candidates"][-num_candidates:]):
                lm_labels = bool(j == num_candidates-1)
                instance, _ = build_input_from_segments(
                                    history, candidate, tokenizer, lm_labels)
...
def train():
    ...
    if args.local_rank in [-1, 0]:
        ...
        checkpoint_handler = ModelCheckpoint(
            tb_logger.writer.log_dir, 'checkpoint', save_interval=1, n_saved=3)
